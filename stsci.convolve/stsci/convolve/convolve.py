...
from . import _correlate, iraf_frame
...
def boxcar(data, boxshape, output=None, mode="nearest", cval=0.0):
    """boxcar computes a 1D or 2D boxcar filter on every 1D or 2D subarray of
    data.

    'boxshape' is a tuple of integers specifying the dimensions of the filter:
    e.g. (3,3)
    ...
    >>> boxcar(np.array([10, 0, 0, 0, 0, 0, 1000]), (3,),
    ...        mode="nearest").astype(np.longlong)
    array([  6,   3,   0,   0,   0, 333, 666], dtype=int64)
    >>> boxcar(np.array([10, 0, 0, 0, 0, 0, 1000]), (3,),
    ...        mode="wrap").astype(np.longlong)
    array([336,   3,   0,   0,   0, 333, 336], dtype=int64)
    >>> boxcar(np.array([10, 0, 0, 0, 0, 0, 1000]), (3,),
    ...        mode="reflect").astype(np.longlong)
    array([  6,   3,   0,   0,   0, 333, 666], dtype=int64)
    >>> boxcar(np.array([10, 0, 0, 0, 0, 0, 1000]), (3,),
    ...        mode="constant").astype(np.longlong)
