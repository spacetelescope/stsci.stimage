...
#define ncoords 512
    ...
    if (find_triangles(
            nunique, ptr1, &ntriangles1, triangles1, max_points, tolerance, max_ratio, &error)) {
        goto exit;
    }

    if (find_triangles(
            nunique, ptr2, &ntriangles2, triangles2, max_points, tolerance, max_ratio, &error)) {
        goto exit;
    }
    ...
    for (i = ntriangles1 - 10; i < ntriangles1; ++i) {
        ...
        printf(
            "   (%.3f, %.3f)--(%.3f, %.3f)--(%.3f, %.3f)\n", tri->vertices[0]->x,
            tri->vertices[0]->y, tri->vertices[1]->x, tri->vertices[1]->y, tri->vertices[2]->x,
            tri->vertices[2]->y);
        printf("   ");
        for (j = 0; j < 3; ++j) {
    ...
    if (merge_triangles(
            ntriangles1, triangles1, ntriangles2, triangles2, &ntriangle_matches, triangle_matches,
            &error)) {
        goto exit;
    }
    ...
    if (ntriangle_matches != ntriangles1) {
        printf(
            "Found %lu instead of %lu when self-matching\n", (unsigned long) ntriangle_matches,
            (unsigned long) ntriangles1);
        goto exit;
    }
    ...
    if (ntriangle_matches != ntriangles1) {
        printf(
            "Found %lu instead of %lu after rejection\n", (unsigned long) ntriangle_matches,
            (unsigned long) ntriangles1);
        goto exit;
    }

    if (vote_triangle_matches(
            ncoords, data2, ncoords, data1, ntriangle_matches, triangle_matches, &ncoord_matches,
            ref_matches, input_matches, &error)) {
        goto exit;
    }

    if (ncoord_matches != max_points) {
        printf(
            "Found %lu instead of %lu after voting\n", (unsigned long) ncoord_matches,
            (unsigned long) max_points);
        goto exit;
    }

    for (i = 0; i < ncoord_matches; ++i) {
        if (ref_matches[i]->x != input_matches[i]->x || ref_matches[i]->y != input_matches[i]->y) {
